...
const TARGET_OPTIONS: { value: TargetType; label: string; description: string }[] = [
  ...
  { value: "students", label: "All Students",    description: "Every active student" },
  { value: "faculty",  label: "All Faculty",     description: "Every instructor" },
  { value: "course",   label: "Course Students", description: "Students enrolled in one course" },
  { value: "user",     label: "Individual User", description: "One specific person" },
];
...
function targetBadge(target: string, courseMap: Map<number, string>, userMap: Map<number, string>) {
  ...
  if (target.startsWith("course:")) {
    ...
    return { label: name ?? `Course #${id}`, color: "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300" };
  }
  if (target.startsWith("user:")) {
    const id = parseInt(target.split(":")[1]);
    const name = userMap.get(id);
    return { label: name ?? `User #${id}`, color: "bg-gray-100 text-gray-700 dark:bg-gray-900/40 dark:text-gray-300" };
  }
            ...
            <form onSubmit={handleSend} className="space-y-4">
              ...
              {(title || message) && (
                <div className="rounded-lg border bg-muted/40 p-3 space-y-0.5">
                  ...
                  <p className="text-sm font-semibold">{title || <span className="italic text-muted-foreground">No title</span>}</p>
